...
final class Migrator
{
    ...
    public function getPendingMigrations(): array
    {
        ...
        return array_values(array_filter($files, fn ($file) => ! in_array($this->getMigrationName($file), $ran, true)));
    }
    ...
    {
        $rows = $this->connection->select('SELECT migration FROM ' . self::MIGRATIONS_TABLE . ' ORDER BY batch ASC, migration ASC');
    ...
    private function getClassFromFile(string $file): string
    {
        ...
        if (preg_match('/class\s+(\w+)\s+extends\s+Migration/', $content, $m) === 1) {
            return $m[1];
        }
    ...
    {
        $this->connection->affectingStatement('DELETE FROM ' . self::MIGRATIONS_TABLE . ' WHERE migration = ?', [$name]);
    }

    private function getNextBatchNumber(): int
    {
        $result = $this->connection->selectOne('SELECT MAX(batch) as max_batch FROM ' . self::MIGRATIONS_TABLE);
